...
es = Elasticsearch(['http://3.34.140.135:9200/'], port=9200, timeout=30, max_retries=10, retry_on_timeout=True)
...
for (root, directories, files) in os.walk(dir_path):
    ...
    for file in files:
        ...
        file_path_split = os.path.join(root, file).split("_")
        indexname = "news-" + file_path_split[1] + "-" + file_path_split[2][0:6]
        print(indexname)
